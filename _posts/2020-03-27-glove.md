...
For example consider two words `ice` and `steam` denoted by $i$ and $j$ resp. Let's analyze co-occurence probabilities with these probe words : `solid`,  `gas`, ``water`, `fashion`.
